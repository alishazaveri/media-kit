...
function DonutChart({
  ...
  const outerR = 58, innerR = 38, cx = 68, cy = 68;
  let angle = -Math.PI / 2;
  const arcs = segments
    .filter((s) => s.pct > 0)
    .map((seg) => {
  ...
  return (
    <svg viewBox="0 0 136 136" className="w-36 h-36 shrink-0">
      ...
      <text x="68" y="63" textAnchor="middle" fill="#0D1B2A" style={{ fontSize: "19px", fontWeight: 900 }}>
        {centerPct}
      </text>
      <text x="68" y="78" textAnchor="middle" fill="#6B7280" style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.06em" }}>
...
export function AudienceSection({
  ...
  const femalePct = gTotal > 0 ? Math.round(((gTotals.F + (gTotals.U ?? 0)) / gTotal) * 100) : 68;
  ...
  const ageGroups = hasData
    ? [
        { label: "Gen Z (18–24)", pct: Math.round((genZ / ageTotal) * 100), color: "#3B5BDB" },
        { label: "Millennial (25–34)", pct: Math.round((millennial / ageTotal) * 100), color: "#0D1B2A" },
        { label: "Alpha (13–17)", pct: Math.round((alpha / ageTotal) * 100), color: "#C8E6E0" },
        { label: "35+", pct: Math.round((older / ageTotal) * 100), color: "#D1D5DB" },
      ].filter((g) => g.pct > 0)
    : [
        { label: "Gen Z (18–24)", pct: 54, color: "#3B5BDB" },
        { label: "Millennial (25–34)", pct: 31, color: "#0D1B2A" },
  ...
  const affinityTags =
    nicheTags.length > 0
      ? nicheTags
      : ["Luxury Tech", "Eco-Apparel", "Clean Beauty", "Digital Art", "Sustainable", "Streetwear"]; // [DUMMY]

  return (
    <section id="audience" className="px-8 py-20 bg-[#E8F5F0]">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-6 items-start">
        ...
        <div className="flex-1 bg-white rounded-3xl p-8 shadow-sm">
            ...
            <div className="space-y-2.5 flex-1">
              {ageGroups.map((g) => (
                <div key={g.label} className="flex items-center justify-between">
                  ...
                  <span className="font-black text-gray-900 text-sm">{g.pct}%</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-4 mb-6">
            <ProgressBar label="FEMALE / NON-BINARY" pct={femalePct} color="#3B5BDB" />
            <ProgressBar label="MALE" pct={malePct} color="#0D1B2A" />
          </div>

          {/* [DUMMY: no device field] */}
          <div className="space-y-4">
            <ProgressBar label="MOBILE" pct={94} color="#3B5BDB" />
            <ProgressBar label="DESKTOP" pct={6} color="#0D1B2A" />
          </div>
        </div>

        {/* Right: dark navy card */}
        <div className="md:w-80 shrink-0 bg-[#0D1B2A] rounded-3xl p-8 text-white">
            ...
            {displayCities.map(({ name, pct }) => (
              <div key={name}>
                ...
                <div className="h-1.5 bg-gray-700 rounded-full overflow-hidden">
                  <div className="h-1.5 bg-blue-500 rounded-full" style={{ width: `${(pct / maxCityPct) * 100}%` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {affinityTags.map((tag) => (
              <span key={tag} className="border border-gray-600 text-white text-xs px-3 py-1.5 rounded-full">
                {tag}
              </span>
            ))}
          </div>
