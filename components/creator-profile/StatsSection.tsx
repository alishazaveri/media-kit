...
export function StatsSection({ stats }: { stats: Stats }) {
  ...
  const ratios = [0, 0.09, 0.19, 0.28, 0.39, 0.51, 0.64, 0.80, 1.0];
  const data = ratios.map((r) => Math.round(startVal + (endVal - startVal) * r));
  const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep"];
  const maxY = Math.ceil((endVal * 1.17) / 350_000) * 350_000;
  const yLabels = [0, 350_000, 700_000, 1_050_000, 1_400_000].filter((v) => v <= maxY + 1);
  const W = 620, H = 160;
  ...
  const metricCards = [
    {
      icon: (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M2 12s3.6-7 10-7 10 7 10 7-3.6 7-10 7-10-7-10-7z" stroke="#6B7280" strokeWidth="1.5" />
        </svg>
      ),
      label: "AVG VIEWS",
    ...
    },
    {
      icon: (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
          <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      label: "AVG LIKES",
    ...
    },
    {
      icon: (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
          <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      label: "COMMENTS",
    ...
    },
    {
      icon: (
        <svg width="13" height="13" viewBox="0 0 24 24" fill="none">
          <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" />
        </svg>
      ),
      label: "SAVES",
  ...
  return (
    <section id="stats" className="px-8 py-20 bg-[#E8F5F0]">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-12 items-start">
        ...
        <div className="md:w-64 shrink-0 space-y-6">
          ...
          {bigStats.map(({ value, label, blue }, i) => (
            <div key={label}>
              {i > 0 && <div className="border-t border-gray-300/50 mb-6" />}
              <p className={`text-5xl font-black leading-none mb-2 ${blue ? "text-blue-600" : "text-gray-900"}`}>
                {value}
              </p>
              <p className="text-xs font-bold tracking-[0.15em] text-gray-400">{label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 bg-white rounded-3xl p-8 shadow-sm">
          <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
            <h2 className="font-black text-gray-900 text-2xl">Follower Growth</h2>
            <div className="flex items-center gap-2">
              ...
              <span className="bg-green-100 text-green-600 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none">
                  <path d="M23 6l-9.5 9.5-5-5L1 18" stroke="#16A34A" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                TRENDING UP {/* [DUMMY] */}
              </span>
            </div>
